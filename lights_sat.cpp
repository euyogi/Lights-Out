...
int main() {
    ...
    // Chamar planejador:
    system("/tmp/dir/software/planners/downward-fdss23/fast-downward.py --alias seq-sat-fdss-2023 --overall-time-limit 895 --plan-file plan.pddl domain.pddl problem.pddl > /dev/null 2>&1");
    remove("domain.pddl");
    remove("problem.pddl");

    int i = 2;
    FILE* plan_buffer = std::fopen("plan.pddl.1", "r"), *plan = plan_buffer;
    while (plan_buffer != NULL) {
        std::string plan_name = "plan.pddl." + std::to_string(i);
        plan_buffer = std::fopen(plan_name.c_str(), "r");
        if (plan_buffer == NULL)
            break;
        plan = plan_buffer;
        ++i;
    ...
    std::cout << std::endl;
    std::fclose(plan);
    system("/tmp/dir/software/planners/downward-fdss23/fast-downward.py --plan-file plan.pddl --cleanup");
}
