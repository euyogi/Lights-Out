...
int main() {
    ...
    for (int i = -1; i <= map_dimension; ++i) {
        for (int j = -1, a = -2; j <= map_dimension; ++j) {
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                if (i >= 0 and i < map_dimension and j >= 0 and j < map_dimension and x >= 0 and x < map_dimension and y >= 0 and y < map_dimension)
                    std::fwrite(predicate.c_str(), predicate.size(), 1, problem);
                std::fwrite(predicate.c_str(), predicate.size(), 1, problem2);
            }
    ...
    // Chamar planejador:
    system("/tmp/dir/software/planners/downward-fdss23/fast-downward.py --alias seq-opt-fdss-2023 --overall-time-limit 850 --plan-file plan.pddl domain.pddl problem.pddl > /dev/null 2>&1");
    FILE* plan = std::fopen("plan.pddl", "r");

    if (plan == NULL) {
        system("/tmp/dir/software/planners/downward-fdss23/fast-downward.py --alias seq-opt-fdss-2023 --overall-time-limit 60 --plan-file plan.pddl domain2.pddl problem2.pddl > /dev/null 2>&1");
