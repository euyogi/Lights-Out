...
int main() {
    ...
    // Chamar planejador:
    system("/tmp/dir/software/planners/madagascar/M -S 1 -Q -t 500 domain2.pddl problem.pddl -o plan.pddl > /dev/null 2>&1");
    ...
    system("/tmp/dir/software/planners/downward/fast-downward.py --alias seq-opt-fdss-2 --overall-time-limit 397 --plan-file plan2.pddl domain.pddl problem.pddl > /dev/null 2>&1");
    FILE* plan2 = std::fopen("plan2.pddl", "r");
    int steps2 = 0;
    if (plan2 != NULL) {
            while ((c  = std::fgetc(plan2)) != EOF) {
                if (c == '(')
                    steps2++;
            }
    }
